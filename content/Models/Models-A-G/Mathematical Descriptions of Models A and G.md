...
Finally, the 'Presence Cube' model demonstrates the constructed vector space on Model A, since $F_{MA}$ and $I$ have size $2^3$, that is faithful to $\mathbb{S}$. This is the Democratic/Aristocratic subgroup, which is isomorphic to $\mathbb{Z}_{2}^3$.  
