...
$\mathcal{D}_i$: This already has been defined. Its group structure is $E_{16}$ (elementary abelian group of order 16), which is also isomorphic to $\mathbb{Z}_2^4$. An object is a dichotomy system ($\mathfrak{D}$, $t$, $\rho$) where $\mathfrak{D}$ is a dichotomy system, $t$ is any valid type of the Socion, and $\rho$ is an $\mathbb{S}$-action 
