...
A set model can be described as a function model represented as a set, with a function being an element of the set. What I mean by an "$E_{16}$ set model" is a set model that precisely models a $C_2^4$ group, which is the vector space associated with the dichotomy system, which acts on these models as they do .
