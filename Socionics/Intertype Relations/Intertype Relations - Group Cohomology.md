...
The map is produced by the cocycles $$f : \mathbb{S} \longrightarrow \mathbf{V}_{i}$$ whose image is denoted by $\mathcal{R}_i$. More rigorously, by taking a cocycle $f \in Z^1(G, A)$ so $f(gh) = f(g) + g \cdot f(h)$. The image subgroup is defined as: $$\mathcal{R}_{i}(f) := \mathrm{Im}(f) = \{f(r) \mid r \in G\} \le$$
