...
When this holds, the type-level transformations and the functional-layout transformations are isomorphic; applying a relation has the same effect as applying it to the layout itself (i.e. in Model L by applying the $A3$ operator to the type ILE will switch the elements in the subgroup of $\{A1, A3\}$, including all of its elements in its cosets in the $\mathcal{R}_i$ space. This also reflects the same action on the functional layout for the ILE in the set model, by switching elements in the same subgroup and elements in the same cosets)
