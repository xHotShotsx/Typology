...
Under the equivariant bijection $\Theta_i$, this corresponds exactly to the same permutation acting on the ILE's functional layout: the functions occupying positions inside that subgroup (and their associated cosets) are swapped in the set model by the action $\rho_{i}(A_{3})$. Thus, $$A_{3} \cdot t \quad \text{and} \quad \rho_{i}(A_{3}) \cdot L_{i}(t)$$implement the same transformation on two di
