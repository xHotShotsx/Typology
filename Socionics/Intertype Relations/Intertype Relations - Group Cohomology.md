...
Equivalently the refinement is the quotient of $T$ by the kernel of the selector signature map, $$\sigma : T \longrightarrow \mathbb{Z}_{2}^{m-1}, \quad t \mapsto (\sigma_{1}(t), \dots, \sigma_{m-1}(t)).$$Thus, $$\mathcal{P}_{i} = \mathrm{ker}$$
 $$$$
Each $\mathcal{P}_i$ corresponds to a unique selector signature and defines a -chotomy group in the dichotomy space $\mathcal{D}_i$.
